    ...
    <div class="content">
        ...
        <div class="row content col-12 menu-sub">
            ...
            <div class="container col filtros">
                <form action="contas_receber.php" method="get" id="busca">
                    <div class="row ">
        ...
        <?php 
            ...
            if (!is_admin()) {
                ...
                ";
                        $q = "UPDATE contas_receber SET situacao = 'Vencida'
                        WHERE situacao = 'À receber' and vencimento < CURDATE()";
                        try {
                            if ($banco->query($q)) {
                                echo "";
                            }
                        ...
                        $q = "SELECT id, data_lancamento, descricao, tipo, id_veiculo, id_motorista, cliente, valor, vencimento, data_recebimento, valor_receb, tipo_receb, saldo, juros_multa, descontos, situacao  
                        FROM contas_receber ";
                        ...
                        if (!$busca) {
                            ...
                            if ($busca->num_rows == 0) {
                                ...
                                while ($reg = $busca->fetch_object()) {
                                    ...
                                    echo "<td>" . numfmt_format_currency($padrao, $reg->juros_multa, 'BRL') . " ";
                                    echo "<td>" . numfmt_format_currency($padrao, $reg->descontos, 'BRL') . " ";
                    
                                    echo "<td>" . numfmt_format_currency($padrao, $reg->valor_receb, 'BRL') . " ";
                                    
                                    echo "<td>$reg->tipo_receb";

                                    if ($reg->data_recebimento == null) {
                                        echo "<td>$reg->data_recebimento";
                                    }
                                    else {
                                        $ajuda = $reg->data_recebimento;
                                        $data = DateTime::createFromFormat('Y-m-d', $ajuda);
                                        $data_recebimento = $data->format('d-m-Y');
                                        echo "<td>$data_recebimento";
                                    }
                                    ...
                                    echo " <a href='contas_receber_receber_form.php?id=$id'> <abbr title='Receber'><i class='bi bi-credit-card'></i></abbr></a>";
                                    }
                                    ...
                                    $soma_juros += floatval($reg->juros_multa);
                                    $soma_desc += floatval($reg->descontos);
                                    $soma_pago += floatval($reg->valor_receb);
                                }
                                ...
                                echo "<td class='total'><td class='total'><td class='total'><td class='total'>";
                            }
                        }
                echo "        
